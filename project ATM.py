...
mydb=mysql.connector.connect(host='localhost',user='root',password='jafer@2',db='college')
mycursor=mydb.cursor()
mycursor.execute('insert into ATM_users values(121122,"vamshi",12345,"savings");')
mycursor.execute('insert into ATM_users values(121123,"akhil",13579,"current");')
mycursor.execute('insert into ATM_users values(121124,"priyanshu ",2468,"savings");')
mycursor.execute('alter table ATM_users add primary key(card_no);')
for x in mycursor:
    print(x)
...
def display_func():
    ...
    message = Message(display_win, text='\n\nYour transaction has been successful\n\nThank you for using our', font=cour20, fg='red')
    message.pack()
...
def question_func():
    ...
    yes_btn = Button(bf, text='YES', font=cour15, fg='yellow', command=balance_func)
...
def balance_func():
    ...
    message.pack()
    text = Label(balance_win, text='services', font=tim40, fg='red')
    text.pack()
...
def enter_pin():
    ...
    note = Label(bf, text='Note:Enter valid pin ', fg='red')
    note.pack()
...
intro = Label(win, text='\nWelcome User ', font=cour20, fg='green')
intro.pack()
...
note = Label(bottom_frame, text='NOTE:DO NOT SHARE YOUR CARD DETAILS', font=cour15, fg='red')
note.pack(pady=10)
